...
namespace WebViewSample
{
	public class InAppBrowserCode : ContentPage
	{
        ...
        {
            this.Title = "Сканирование марок";
            var layout = new StackLayout { Margin = new Thickness(20) };
			var controlBar = new StackLayout (){ Orientation = StackOrientation.Horizontal };
			var backButton = new Button{ Text = "Назад", HorizontalOptions = LayoutOptions.StartAndExpand };
			backButton.Clicked += backButtonClicked;

            var reloadButton = new Button { Text = "Обновить", HorizontalOptions = LayoutOptions.CenterAndExpand };
            reloadButton.Clicked += OnReloadButtonClicked;

			var forwardButton = new Button{ Text = "Вперёд", HorizontalOptions = LayoutOptions.EndAndExpand };
